...
def count_track_decade(df, decade):
    """Cuenta el número de canciones de los albumes publicados en
    una década especifica
...
def most_popular_track(df, year):
    """ Devuelve una lista con las canciones más populares en los
    últimos x años

    :param df: Dataframe incluyendo tracks, artists y albums
    :param year: Número de años (ej: 10)
    :return: Devuelve una lista con las canciones más populares de los
    últimos años
    """

    tracks = df[df['album_release_year'].between(todays_year - year,
                                                 todays_year)]
    mp = tracks[tracks['popularity'] == max(tracks['popularity'])]['name']
...
def active_artists(df, decade):
    ...
    """
    if decade % 10 == 0:
        df['decade'] = [year - (year % 10) for year
                        in df["album_release_year"]]
        ...
        if len(artists) == 0:
            ...
            return print("Active artists since {}'s decade:".format(decade),
                         selected)
    else:
        return print('Please, select a correct decade (e.g. 1970, 1990, 2000)')
